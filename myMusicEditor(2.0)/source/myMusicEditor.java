...
public class myMusicEditor {
    ...
    public musicNote checkSelected(int x, int y) {
        musicNote r;

        for (musicNote n : mv.allNotes) {
            if ((n.x < x) && ((n.x+ n.width) > x)) {
                if ((n.y < y) && ((n.y+ n.height) > y)) {
                    r = n;
                    return r;
                }
            }
    ...
    myMusicEditor() {
        ...
        MouseAdapter ma = new MouseAdapter() {
            ...
            public void mousePressed(java.awt.event.MouseEvent e) {
                ...
                if (drawMode) {
                    selectN = new musicNote(determineImagePath(), e.getX(), e.getY());
                    mv.setDeleteTarget(selectN);
                    mv.addNote(selectN);

                    mv.revalidate();
                    mv.repaint();
                } else if (selectMode) {
                    selectN = checkSelected(e.getX(), e.getY());
                    mv.delTarget = selectN;

                    if (selectN != null) {
                        xOff = e.getX()-selectN.x;
                        yOff = e.getY()-selectN.y;
                        if (selectN.m != null) {
                            statusL.setText(selectN.getPitch());
                        }
                    }
            ...
            public void mouseDragged(java.awt.event.MouseEvent e) {
                if (drawMode) {
                    ...
                    if (selectN != null) {
                        selectN.setX(e.getX() - xOff);
                        selectN.setY(e.getY() -yOff);
                    }
                    contentArea.revalidate();
                    contentArea.repaint();
                }
                ...
                if (selectN != null) {
                    ...
                    mv.setNoteStaff(selectN);
                    if (selectN.m != null) {
                        selectN.determinePitch(selectN.circleCenterY-(selectN.m.firstLine-10));
                        statusL.setText(selectN.getPitch());
                    }
                }
                contentArea.revalidate();
